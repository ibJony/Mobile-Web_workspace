...
public class PollCodeActivity extends Activity implements OnItemSelectedListener {
			...
			// JSON Node names
						...
						//polls results returned
						List<Integer> rid = new ArrayList<Integer>();
						List<Integer> rtypid = new ArrayList<Integer>();
						...
						String total,totalyes,totalno,theQn,p;
	...
	class LoadAllPolls extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String file_url) {
			...
			ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(PollCodeActivity.this,
					android.R.layout.simple_spinner_item, plabcod);
	...
	class LoadPollResults extends AsyncTask<String, String, String> {
			...
			try {
				...
				if (success == 1) {
					...
					for (int i = 0; i < pollresults.length(); i++) {
						...
						reid = catObj.getInt("poll_id");
						retype = catObj.getInt("poll_typeid");
						resp = catObj.getInt("poll_response");
						String cont = catObj.getString("poll_comment");
						p = String.valueOf(resp);
		...
		protected void onPostExecute(String file_url) {
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					//finish();
					Toast.makeText(getApplicationContext(),pid+" "+polltype_id+" "+labresp.size()+" "+labrespyes.size(),Toast.LENGTH_LONG).show();
				}
			});
		...
		public void pollcount()
		{
			...
			totalno = String.valueOf(labrespno.size());
			
			//var2 = Integer.parseInt(totalno);
			//var2= totalvar-var1;
